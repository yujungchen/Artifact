...
#undef max
    ...
    printf("Scene X range[%f %f]\n", minX, maxX);
    printf("Scene Y range[%f %f]\n", minY, maxY);
    printf("Scene Z range[%f %f]\n", minZ, maxZ);
    ...
    // Walk through the vertex array and update the positions
     for (int i = 1; i < model->numvertices+1; i++) {
		...
		minX = std::min(minX, model->vertices[i * 3]);
        maxX = std::max(maxX, model->vertices[i * 3]);
        minY = std::min(minY, model->vertices[i * 3 + 1]);
		maxY = std::max(maxY, model->vertices[i * 3 + 1]);
        minZ = std::min(minZ, model->vertices[i * 3 + 2]);
        maxZ = std::max(maxZ, model->vertices[i * 3 + 2]);
    }

    printf("Unitized Scene X range[%f %f]\n", minX, maxX);
    printf("Unitized Scene Y range[%f %f]\n", minY, maxY);
    printf("Unitized Scene Z range[%f %f]\n", minZ, maxZ);
    ctrX = 0.5*(minX+maxX);
    ctrY = 0.5*(minY+maxY); 
    ctrZ = 0.5*(minZ+maxZ);
